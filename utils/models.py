...
class ConvCritic(nn.Module):
    def __init__(self, input_size=500, lead_n=12, label_size=7, hidden_size=256, device='cpu'):
        ...
        self.conv = nn.Sequential(
            nn.Conv1d(in_channels=self.lead_n, 
                      ...
                      dilation=2, 
                      groups=self.lead_n),
            nn.BatchNorm1d(self.lead_n * 4, track_running_stats=False),
            nn.AvgPool1d(kernel_size=2),
            nn.Conv1d(in_channels=self.lead_n * 4, 
                      out_channels=self.lead_n * 4, 
                      kernel_size=7, 
                      stride=2, 
                      padding=3, 
        ...
        conf_out_size = int(self.lead_n * 6 * third_l_output)
...
class InverseCNNGenerator(nn.Module):
    def __init__(self, 
        ...
        self.deconv = nn.Sequential(
            ...
            nn.BatchNorm1d(self.lead_n * 2, track_running_stats=False),
            nn.PReLU(),
        ...
        self.encoder = nn.Sequential(
            nn.PReLU(),
            nn.Linear(in_features=self.label_size + self.noise_size, out_features=self.hidden_size)
        ).to(self.device)
...
class LSTMGenerator(nn.Module):
    def __init__(self, label_size, hidden_size, noise_size, n_lead=12, device='cpu'):
...
#         self.output_size = output_size
        ...
        self.out = nn.Sequential(
#             nn.BatchNorm1d(self.hidden_size * self.lstm_output_mult + self.hidden_size, track_running_stats=False),
            nn.PReLU(),
            nn.Linear(self.hidden_size * self.lstm_output_mult + self.hidden_size, self.n_lead, bias=False)
        ).to(self.device)

    def init_hidden(self, batch_size):
        return tuple([torch.zeros((4, batch_size, self.hidden_size), dtype=torch.float32).to(self.device)
                      for _ in range(2)])
...
#         print(encoded_labels.unsqueeze(1).shape)
        x = torch.cat([encoded_seq, encoded_labels.unsqueeze(1).expand(-1,noise.size(1), -1)], dim=2)
#         print(x.shape)
...
class LSTMCritic(nn.Module):
    def __init__(self, input_size, label_size, hidden_size, lead_n, device='cpu'):
        ...
        self.lstm = nn.Sequential(
            nn.LSTM(input_size=self.lead_n,
                    hidden_size=self.hidden_size,
                    num_layers=2,
                    batch_first=True,
                    dropout=0.2,
        ...
        self.size_decrease = nn.Sequential(
            nn.BatchNorm1d(self.input_size, track_running_stats=False),
            nn.AvgPool1d(kernel_size=8),
        ).to(self.device)
                                        
        self.out = nn.Sequential(
            nn.Linear(self.hidden_size * self.input_size // 4, self.hidden_size),
            nn.BatchNorm1d(self.hidden_size, track_running_stats=False),
            nn.PReLU(),
    ...
    def init_hidden(self, batch_size):
        return tuple([torch.zeros((2, batch_size, self.hidden_size), dtype=torch.float32).to(self.device) for _ in range(2)])

    def forward(self, seqs, labels):
        x, hidden = self.lstm(seqs)
#         print(x.shape)
        x = self.size_decrease(x)
#         print(x.shape)
        return self.out(x.contiguous().view(x.size(0), -1))
